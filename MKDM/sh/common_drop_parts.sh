#!/bin/ksh
#*******************************************************************************
#** Program         :  common_drop_parts.sh
#**
#** Job Name        : 
#**
#** Original Author :  Keerthana Raman
#**
#** Description     :  This job drops partitions for tables in all modules
#**                    
#**                    
...
get_part_mod_list DROP_PART_MOD_LIST PART_MODULE_LIST
check_status
...
if [ $start_step -le $step_number ] ; then
 ...
 #Write in header file
       echo '                                                                             ' >$HEADER
       echo '*---------------------------------------------------------------------------*'>>$HEADER
       echo '*                      DROP PARTITION PROCESS REPORT                        *'>>$HEADER
       echo '*---------------------------------------------------------------------------*'>>$HEADER
       echo '*  TABLE_NAME                 | OWNER |LST_PRT_DRP_DT|    STATUS            *'>>$HEADER
       echo '*---------------------------------------------------------------------------*'>>$HEADER
    rm $HOME/*_drop_part_rpt.txt
...
if [ $start_step -le $step_number ] ; then
     ...
     # Write in module status file 
        echo '                                                                             ' >$MODSTATUS
        echo '*----------------------------------------------------------------*'>>$MODSTATUS
        echo '*      MODULE WISE REPORT FOR DROP PARTITION PROCESS             *'>>$MODSTATUS
        echo '*----------------------------------------------------------------*'>>$MODSTATUS
        echo '*     MODULE NAME    |       STATUS                              *'>>$MODSTATUS
        echo '*----------------------------------------------------------------*'>>$MODSTATUS
     rm $HOME/*_drop_main_rpt.txt
fi
...
echo "Connect variable :::::::: $ORA_CONNECT"
			#-----------------------------------------------------------------
			step_number=4
			# Description: Drop partitions for tables in corresponding module         
			#-----------------------------------------------------------------
			if [ $start_step -le $step_number ] ; then
			    echo "*** Step Number $step_number"
			    run_sql drop_partition.sql $MODULE_CODE
			    check_status
			fi
			...
			#-----------------------------------------------------------------
			step_number=6
			#Description: Delete data for non-partitioned tables
			#-----------------------------------------------------------------
			if [ $start_step -le $step_number ] ; then
			   echo "*** Step Number $step_number"
			   run_sql del_hist_data.sql $MODULE_CODE
			   check_status
			fi			
			...
			if [ $start_step -le $step_number ] ; then
			    echo "*** Step Number $step_number"
			    RPTFILE=$HOME/${MODULE_CODE}_drop_part_rpt.txt
			    run_sql drop_part_rpt.sql $RPTFILE $MODULE_CODE 
			    check_status
			...
			if [ $start_step -le $step_number ] ; then
			    echo "*** Step Number $step_number"
			    ORA_CONNECT=$CONNECT_MKDM
                            run_sql check_drop_partition.sql $CHKFILE $MODULE_CODE
                            check_status
...
if [ $start_step -le $step_number ] ; then
    echo "*** Step Number $step_number"
    cd $HOME
    for fl_nm in *_drop_part_rpt.txt
    do
    MAIN_MOD_NM=`echo $fl_nm | cut -d'-' -f1`
    cat $fl_nm >> $HOME/${MAIN_MOD_NM}_drop_main_rpt.txt
    done

    for fl_nm in *_drop_main_rpt.txt
    do
    ...
    subject_msg="Drop partition process for $MAIN_MOD_NM"
...
#-----------------------------------------------------------------
step_number=10
# Description: Check for any errors in partition Dropping
#-----------------------------------------------------------------
if [ $start_step -le $step_number ] ; then
    echo "*** Step Number $step_number"
    cd $HOME
    ...
    run_sql check_drop_partition.sql $CHKFILE $MAIN_MOD_CD
    check_status
    export check_run=`cat $CHKFILE`
    if [ $check_run -gt 0 ] ;  then
       ...
       check_status
        ...
        check_status
        rm $HOME/$fl_nm
        rm $HOME/${MAIN_MOD_CD}*drop_part_rpt.txt
    fi
    done
    cd -
fi
...
if [ $start_step -le $step_number ] ; then
    ...
    if [[ ${mod_in_error} -gt 0 ]]; then
        subject_msg="Common partition Drop process Failed"	
        send_mail "$success_msg" "$subject_msg" "$COMMON_PART_MAIL" 
        check_status
        exit 3 
    else        
        subject_msg="Common partition Drop process completed Successfully."
        send_mail "$success_msg" "$subject_msg" "$COMMON_PART_MAIL"
        check_status
    fi
fi
